...
for (const testCase of CASES) {
  try {
    ...
    const actualToolNames = response.toolCalls.map(
      (call) => call.payload.toolName,
    );
    const passed =
      testCase.expectedToolNames.length === actualToolNames.length &&
...
console.info(
  `=== Lesson 02 汇总：通过 ${passedCount}/${CASES.length}；失败 ${failedCount}/${CASES.length} ===`,
);
