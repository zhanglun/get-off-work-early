...
打开 Demo
  ...
  → 点击“新建项目”
  → 直接进入项目主对话
  → 助手邀请用户粘贴或上传完整剧本（可一份包含多集）
```

项目名称和集数优先由助手从剧本中识别；缺失时在对话中补问，不使用独立创建表单。

完整剧本由助手自动拆分成集后再开始制作（拆集策略见 §5.1）；镜头数量不再人工指定，由模型按剧本内容规划并校验（见 §6）。
...
导入一份包含多集的完整剧本时，助手自动拆分成集，策略为「规则优先、模型辅助、双重校验」：

1. 规则拆分：按「第X集 / 第X话 / EP几 / Episode 几」等分集标题切分原文；
2. 模型校验：规则拆分成功后，由模型审查员核对分集边界的剧情完整性（每集独立成集、无跨集截断）；
3. 模型辅助：规则识别不到分集标记时，由模型基于场次序列做分组拆分；
4. 结构校验：模型分组必须构成对全部场次的完整划分（不重不漏、集数递增），不通过则携带反馈重试，仍失败则显式报错，不静默降级；
5. 模型复查：拆分完成后模型审查员复查，发现错误时给出修正分组，修正结果重新过结构校验。

拆分结果以结构化「拆分确认卡」在对话中呈现（每集集数、标题、场次、字数、梗概与审查提示），用户点卡片按钮或回复确认后批量登记各集并依次排队制作；顶栏提供集数状态条（已登记/制作中/已完成/失败），点击可切换当前查看的剧集，默认跟随正在制作的一集。登记与制作仍遵守「同项目一次只制作一集」。
...
- 镜头数量由模型按每集场次内容与节拍密度规划，经程序校验（每场镜头数合理、总数在有效区间、场次全覆盖）；校验不通过携带反馈重试，仍失败显式报错，不静默降级。
