...
> PG 是我们的持久化数据库——任务状态、Prompt、Token 统计、用户权限都存在 PG（NestJS 主项目和 Go 任务控制面都用 PG）。本章结合项目讨论索引、事务、MVCC、Explain、优化，不背概念。
>
> 本章共 14 题。
...
> 本章开头提到 PG 用 WAL 保证持久化。WAL 到底是怎么工作的？为什么不直接写数据文件？
...
1. **崩溃恢复（Crash Recovery）**：WAL 落盘了就算"提交成功"，dirty page 哪怕没刷盘也能从 WAL 重放。这正是本章开头说"任务状态落 PG = WAL 落盘 = 持久化"的底层。
