...
### 🕳️ 踩坑 5：工具缺口导致幻觉（能力边界 = 工具边界）

**现象**：让 Agent "查看一下当前目录的文件夹和文件信息"，它**硬用 `read_file(path='.')` 去读目录**，报 `IsADirectoryError`，然后还**编造了一个空答案**："当前目录下包含以下文件和文件夹："（后面什么也没有）。

**原因**：Agent 手上只有 `add` / `read_file` / `search_web` 三个工具，**没有"列目录"的能力**。LLM 知道用户想要什么，但没有合适的工具可选，只能硬套最接近的 `read_file`——失败后还不肯承认不知道，开始编。这是典型的**能力缺口 → 工具误用 → 幻觉**链路。

**解决**：新增 `list_dir` 工具，三处改动：
- `tools.py`：写 `list_dir` 函数（`os.scandir`，返回结构化列表），顺手让 `read_file` 遇到目录时明确提示"请改用 list_dir"
- `schemas.py`：加 schema，description 里写明"看目录用这个，读文件用 read_file"
- `agent.py`：注册到 `TOOL_REGISTRY`

**教训**：**Agent 的能力上限由工具决定，但工具的"好用程度"由 description 和错误提示决定。** 同样是加工具，真正让它好用的是两件看不见的事——schema description 教 LLM 怎么选，错误信息引导 LLM 自我纠正。详见思考 5。
...
### 思考 5：完备的 Agent ≠ 工具多的 Agent（一次真实踩坑引发的思考）

Day 4 实跑时遇到一个真实场景：让 Agent "查看一下当前目录的文件夹和文件信息"，结果它**硬用 `read_file(path='.')` 去读目录**，报 `IsADirectoryError`，然后还**编造了一个空答案**（典型幻觉）。

根因很清楚：Agent 手上只有 `add` / `read_file` / `search_web` 三个工具，**没有"列目录"的能力**。LLM 知道用户想要什么，但没有合适的工具可选，只能硬套最接近的那个。

修复方式是新增一个 `list_dir` 工具。但这次踩坑引出一个比"加工具"更深的认知：

> **"Agent 需要集成各种各样的工具才完备"——这个直觉对了一半。**

**对的部分**：工具决定 Agent 的能力边界。没有 `list_dir`，再聪明的 LLM 也列不出目录。这句没错。

**误导的部分**：工具不是越多越完备。一个有 100 个工具但乱用的 Agent，往往不如一个有 3 个工具但用得精准的 Agent。

| 维度 | 工具太多反而有害 |
|------|----------------|
| **决策变慢** | LLM 每次都要从一堆工具里挑，token 消耗大、延迟高 |
| **选择困难** | 工具语义重叠时（`read_file` / `cat_file` / `view`），LLM 容易选错 |
| **上下文爆炸** | 每个工具 schema 都要塞进 prompt，工具一多，留给对话的空间就少 |
| **维护成本** | 工具越多，越容易出 bug、越难测试 |

**真正的"完备"是三件事的平衡**：

```
1. 工具覆盖度    ← "目标场景需要的能力是否齐全"
2. LLM 决策能力  ← "能不能在合适时机选对工具"（更重要）
3. 工具设计质量  ← "description / 错误提示是否教 LLM 正确使用"
```

**最有反直觉的一点：第 2、3 项往往比第 1 项更重要。** 同样是加 `list_dir`，真正让它好用的不只是函数本身，而是两件"看不见但关键"的细节：

1. 在 schema 的 `description` 里写明"看目录用这个，读文件用 `read_file`"——**教 LLM 怎么选**
2. 让 `read_file` 遇到目录时返回"请改用 `list_dir`"——**错误时引导 LLM 自我纠正**

这两件事让 Agent 即使工具不多，也能精准决策、甚至从错误中恢复。比"多加工具"本身更值钱。

**业界的两种范式都成立**：

| 范式 | 例子 | 特点 |
|------|------|------|
| **窄而深** | Cursor（写代码）、Devin（软件工程） | 工具少（几个到十几个），但每个打磨到极致，决策逻辑极强 |
| **广而全** | ChatGPT 插件、GPTs Store、Manus | 工具多（上百个），靠生态覆盖，但单个任务未必精 |

商业上**"窄而深"更容易做出好产品**——Agent 在自己擅长的领域，工具不需要多，但 LLM + 工具的配合要打磨到位。米其林大厨的刀具可能比你少，但每把都用得登峰造极。

**这个思考直接指向 Lesson 07（MCP）**：与其每个 Agent 都自己集成一堆工具，不如把工具标准化、协议化，让任何 Agent 都能即插即用任何工具。认知升级路线：

```
Day 4：   "Agent 需要工具"              ← 现在的阶段 ✓
Day 5：   "Agent 需要按场景配工具（少而精）"
Lesson 07 (MCP)："工具和 Agent 解耦"    ← 工具是独立生态
```

> 🔑 一句话总结：**完备的 Agent ≠ 工具多的 Agent。完备 = "少量高质量工具 + 强决策 + 优雅错误处理" 覆盖目标场景。工具决定下限，决策决定上限。**
